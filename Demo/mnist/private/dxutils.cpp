...
namespace DXUtils
{
    ...
    unsigned int SizeOfFormat(DXGI_FORMAT format, mnist::TLogFn logFn)
    {
        switch(format)
        {
        ...
        logFn((int)mnist::LogLevel::Error, "Unhandled DXGI_FORMAT");
        return 0;
    }

    // How many channels the pixel format has
    unsigned int FormatChannelCount(DXGI_FORMAT format, mnist::TLogFn logFn)
    {
        switch(format)
        {
            case DXGI_FORMAT_R8_UNORM:
    ...
    DXUtils::FormatChannelType GetFormatChannelType(DXGI_FORMAT format, mnist::TLogFn logFn)
    {
        switch(format)
        {
            case DXGI_FORMAT_R8_UNORM:
    ...
    UploadBufferTracker::Buffer* UploadBufferTracker::GetBuffer(ID3D12Device* device, size_t size, mnist::TLogFn logFn, bool align256)
    {
        ...
        else
        {
            ...
            if (FAILED(hr))
            {
                logFn((int)mnist::LogLevel::Error, "Failed creating a resource to upload shader constants.");
                return nullptr;
            }
        }
    ...
    bool CreateHeap(Heap& heap, ID3D12Device* device, int numDescriptors, D3D12_DESCRIPTOR_HEAP_TYPE type, D3D12_DESCRIPTOR_HEAP_FLAGS flags, mnist::TLogFn logFn)
    {
        ...
        {
            logFn((int)mnist::LogLevel::Error, "Could not create heap");
            return false;
        }
    ...
    ID3D12Resource* CreateTexture(ID3D12Device* device, unsigned int size[3], DXGI_FORMAT format, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES state, ResourceType textureType, LPCWSTR debugName, mnist::TLogFn logFn)
    {
        ...
        if (FAILED(hr))
        {
            logFn((int)mnist::LogLevel::Error, "Could not create texture");
            return nullptr;
        }
    ...
    ID3D12Resource* CreateBuffer(ID3D12Device* device, unsigned int size, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES state, D3D12_HEAP_TYPE heapType, LPCWSTR debugName, mnist::TLogFn logFn)
    {
        D3D12_HEAP_PROPERTIES heapDesc = {};
        ...
        if(FAILED(hr))
        {
            logFn((int)mnist::LogLevel::Error, "Could not create buffer");
            return nullptr;
        }
    ...
    bool CopyConstantsCPUToGPU(UploadBufferTracker& tracker, ID3D12Device* device, ID3D12GraphicsCommandList* commandList,ID3D12Resource* resource, void* data, size_t dataSize, mnist::TLogFn logFn)
    {
        ...
        mnist::TLogFn logFn)
    {
        ...
        if (FAILED(hr))
        {
            ...
            logFn((int)mnist::LogLevel::Error, "Could not serialize root signature: %s", errorMsg);
            if(sig) sig->Release();
        ...
        {
            const char* errorMsg = (error ? (const char*)error->GetBufferPointer() : nullptr);
            logFn((int)mnist::LogLevel::Error, "Could not create root signature: %s", errorMsg);
            if(sig) sig->Release();
            if(error) error->Release();
            return false;
        }
    ...
    inline void MakeDescriptorTable(ID3D12Device* device, Heap& srvHeap, const ResourceDescriptor* descriptors, size_t count, mnist::TLogFn logFn)
    {
        size_t startIndex = srvHeap.nextIndexFree;
        srvHeap.nextIndexFree += count;
        if (srvHeap.nextIndexFree > srvHeap.indexCount)
        {
            logFn((int)mnist::LogLevel::Error, "Ran out of descriptors, please increase c_numSRVDescriptors");
            return;
        }
        ...
        for (size_t index = 0; index < count; ++index)
        {
            ...
            {
                logFn((int)mnist::LogLevel::Error, "Unhandled type encountered");
            }
        }
    }

    D3D12_GPU_DESCRIPTOR_HANDLE GetDescriptorTable(ID3D12Device* device, Heap& srvHeap, const ResourceDescriptor* descriptors, size_t count, mnist::TLogFn logFn)
    {
